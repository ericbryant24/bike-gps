...
- **Honest time estimates**: BRouter's physics model assumes a rider who never stops; the app adds expected waits at every traffic light (~25 s) and stop sign (~6 s) on the route, and a *Riding pace* setting (Relaxed / Moderate / Brisk ≈ 65 / 85 / 110 W) sets the effort the model assumes, hills still counted. The route sheet shows the split, "Riding 20 min + ~4 min at 9 lights and 2 stop signs", and the pace chip next to it cycles the pace with a tap.
- **Route sheet**: the collapsed peek shows the stats, the timing split and Start; pull it up for a segmented control — *Overview* (bike-friendliness composition, warnings, bike racks), *Routes* (the profile to prefer plus the alternative routes side by side) and *Steps* (turn-by-turn with per-road grades; tap a step to pan the map to it). Share sits in the header; Simulate and GPX export live under ⋯.
...
- **Saved places**: save any place from its card ("☆ Save place") or a planned destination (⋯ → "Save destination"), name it (Home / Work quick picks), and it appears as a ★ pin on the map, first in search (by its name or the original place name) and in the empty search box, and under ★ Saved places in the menu, where you can rename, show or delete. Stored on the device.
...
- **Splash screen**: the logo on brand teal from the first frame until the map is ready (an in-page overlay), plus pre-rendered iOS launch images for each iPhone screen size (`splash/`, generated by `scripts/make-splash.mjs`) so the installed app no longer opens to a white screen.
...
js/
  ...
  favorites.js        saved places (pure helpers; persisted by main.js)
